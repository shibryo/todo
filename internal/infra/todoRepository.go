...
func (t *TodoRepository) FindAll() ([]*domain.Todo, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to convert to todo models: %w", err)
	}
...
func convertToTodoModels(todos []*Todo) ([]*domain.Todo, error) {
	...
	for _, todo := range todos {
		...
		lastUpdate := domain.NewLastUpdate(domain.NewLastUpdate(domain.NewTime(todo.LastUpdate)))
		createdAt := domain.NewCreatedAt(domain.NewCreatedAt(domain.NewTime(todo.CreatedAt)))
...
func (t *TodoRepository) FindByID(id uint64) (*domain.Todo, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to convert to todo model: %w", err)
	}
